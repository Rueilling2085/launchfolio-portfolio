...
const COPY = {
  infoArchitectureTitle: { zh: "資訊架構圖", en: "Information Architecture" },
  infoArchitectureLegend: { zh: "紫色標示為下方說明的核心功能與畫面", en: "Purple marks the core features and screens described below" },
  emotionShareLabel: { zh: "情感互動分享", en: "Emotional interaction & sharing" },
} as const;
...
function WidgetCard({ widget, lang }: { widget: FeatureWidget; lang: "zh" | "en" }) {
  return (
    <Image
      src={widget.image}
      alt={widget.imageAlt?.[lang] ?? ""}
      width={widget.width}
...
function ScreenMedia({ screen, lang }: { screen: FeatureScreen; lang: "zh" | "en" }) {
  return screen.bare ? (
    <Image
      src={screen.image}
      alt={screen.imageAlt?.[lang] ?? screen.title[lang]}
    ...
    <PhoneFrame image={screen.image} imageAlt={screen.imageAlt?.[lang] ?? screen.title[lang]} />
  );
}

function DescriptionBlock({ screen, lang }: { screen: FeatureScreen; lang: "zh" | "en" }) {
  ...
  return (
    <div className="w-full max-w-sm shrink-0 text-center md:w-[260px] md:text-left">
      ...
      <p className="mt-4 text-lg font-semibold text-ink">{screen.title[lang]}</p>
      <p className="mt-2 text-sm leading-relaxed text-muted">{screen.description[lang]}</p>
    </div>
  );
}

function FeatureRow({ screen, lang }: { screen: FeatureScreen; lang: "zh" | "en" }) {
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <div id="information-architecture" className="scroll-mt-24">
          <RevealOnScroll delay={0.05} className="mt-14 md:mt-20">
            <h3 className="mb-6 text-center text-sm font-medium text-white/50">{COPY.infoArchitectureTitle[lang]}</h3>
            <InformationArchitecture legend={COPY.infoArchitectureLegend[lang]} />
          </RevealOnScroll>
        </div>
          ...
          >
            <div className="relative flex flex-col gap-16 md:gap-24">
              {data.screens.map((screen, i) => (
                <RevealOnScroll key={screen.title.zh} delay={0.05 * i}>
                  <FeatureRow screen={screen} lang={lang} />
                </RevealOnScroll>
              ))}
            </div>
          </div>
        ...
        {data.sceneShowcase && (
          <RevealOnScroll delay={0.05} className="mt-14 md:mt-20">
            <div className="relative">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
                <div className="relative aspect-[4/5] overflow-hidden rounded-3xl border border-white/10 bg-[#120A2E]">
                  <Image
                    ...
                    alt={data.sceneShowcase.appInHand.imageAlt?.[lang] ?? ""}
                    fill
                ...
                <div className="relative aspect-[4/5] overflow-hidden rounded-3xl border border-white/10">
                  <Image
                    ...
                    alt={data.sceneShowcase.concertPhoto.imageAlt?.[lang] ?? ""}
                    fill
              ...
              <div
                aria-label={COPY.emotionShareLabel[lang]}
            ...
            {data.sceneShowcase.caption && (
              <p className="mx-auto mt-6 max-w-md text-center text-sm leading-relaxed text-white/70 md:mt-8 md:text-base">
                {data.sceneShowcase.caption[lang]}
              </p>
            )}
            ...
            <div className="relative mt-10 aspect-[16/9] overflow-hidden rounded-3xl border border-white/10 md:mt-14">
              <Image
                ...
                alt={data.sceneShowcase.hero.imageAlt?.[lang] ?? ""}
                fill
        ...
        {data.deviceConnection && (
          <RevealOnScroll delay={0.05} className="mt-14 md:mt-20">
            {data.deviceConnection.title && (
              ...
              >
                {data.deviceConnection.title[lang]}
              </h3>
            )}
            <div className="grid grid-cols-1 items-stretch gap-4 md:grid-cols-2 md:gap-8">
              <div className="relative aspect-[4/5] w-full self-start overflow-hidden rounded-3xl border border-white/10 bg-[#120A2E]">
                <Image
                  ...
                  alt={data.deviceConnection.render.imageAlt?.[lang] ?? ""}
                  fill
