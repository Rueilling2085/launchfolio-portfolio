...
function StructureColumn({ column }: { column: SurveyResearchData["structure"][number] }) {
  return (
    <div className="w-full flex-1 rounded-2xl border border-line bg-white p-5">
      <p className="text-xs font-semibold text-[#045CC4]">{column.label}</p>
    ...
    <div className="rounded-2xl border border-line bg-white p-5">
      ...
      {finding.stat && (
        <p className="mt-3 leading-none">
          ...
          <span className="ml-2 text-xs text-muted">{finding.stat.label}</span>
        </p>
      )}
      <p className="mt-3 text-sm leading-relaxed text-muted">{finding.description}</p>
    </div>
  );
}
    ...
    <div className="relative mt-16 md:mt-24">
      ...
      <RevealOnScroll delay={0.08} className="mt-6">
        <div className="grid grid-cols-1 items-center gap-8 md:grid-cols-[1fr_320px] md:gap-14">
          <p className="whitespace-pre-line text-sm leading-relaxed text-muted md:text-base">{data.goal}</p>
          ...
          <div className="flex flex-col gap-4">
            {data.stats.map((stat) => (
              ...
              >
                ...
                <p className="mt-3 text-xs font-medium text-muted md:text-sm">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {data.motivation ? (
          ...
          <div className="flex flex-col items-stretch gap-3 lg:flex-row lg:items-center">
            {data.structure.map((column, i) => (
              <div key={column.label} className="flex flex-col items-center gap-3 lg:contents">
      ...
      {data.findings.length > 0 && (
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
          {data.findings.map((finding, i) => (
            <RevealOnScroll key={finding.title} delay={0.05 * i}>
